...
uses
  ...
  Dialogs, StdCtrls, Buttons, ExtCtrls, gtPDFViewer, ComCtrls,
  {gtconsts, }gtPDFPrinter, gtPDFOutlineViewer, //gtClasses,
  gtCstPDFDoc, gtExPDFDoc, gtExProPDFDoc, gtPDFDoc, //gtPDFCrypt,
  gtPDFSearchPanel, ImgList, System.ImageList, gtPDFClasses,
  gtScrollingPanel;
...
type
  TfrmPDFViewerDemo = class(TForm)
    ...
    procedure AbrirFichero(Nombre:String);
...
{    procedure gtPDFDocument1Password(Sender: TObject; var Password: String;
    ...
    procedure gtPDFViewer1ViewerChange(Sender: TObject;
...
//------------------------------------------------------------------------------
procedure TfrmPDFViewerDemo.Inicializa;
var LBmp: TBitmap;
begin
  if gtPDFDocument1.IsLoaded =False
   then exit;

  gtPDFViewer1.Zoom   := 75;
...
//------------------------------------------------------------------------------
procedure TfrmPDFViewerDemo.AbrirFichero(Nombre:String);
begin
  gtPDFDocument1.Filename := Nombre;
  gtPDFDocument1.LoadFromFile(Nombre)
end;
//------------------------------------------------------------------------------
...
begin
  if not OpenDialog1.Execute then
    Exit;
  AbrirFichero (OpenDialog1.FileName);
  Inicializa;
end;
//------------------------------------------------------------------------------
...
procedure TfrmPDFViewerDemo.gtPDFViewer1ViewerChange(Sender: TObject;
  var Continue: Boolean);
begin
  Continue := True;
  UpdateButton;
...
procedure TfrmPDFViewerDemo.FormShow(Sender: TObject);
begin
  Inicializa;
end;
